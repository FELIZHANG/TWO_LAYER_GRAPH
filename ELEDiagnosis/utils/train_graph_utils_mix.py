...
class train_utils(object):
    ...
    def setup(self):
        ...
        if args.task == 'Node':
            self.model = getattr(models, args.model_name)(feature=feature,out_channel=9)
        elif args.task == 'Graph':
            # self.model = getattr(models2, args.model_name)(feature=feature, out_channel=9,pooltype=args.pooltype)
...
def plot_matrix(y_true,y_pred,labels_name,title=None,thresh=0.8,axis_labels=None):
    ...
    plt.imshow(cm,interpolation='nearest',cmap=plt.get_cmap('spring'))
    plt.colorbar()
