...
def Gen_graph(graphType, data, label,task):
    data_list = []
    #三种判断是否为邻居的方式KNNGraph;RadiusGraph;PathGraph
...
def visualize_graph(graph):
    ...
    for edge in range(graph.edge_index.shape[1]):
        source = graph.edge_index[0, edge].item()
        target = graph.edge_index[1, edge].item()
        G.add_edge(source, target, weight=graph.edge_attr[edge].item())

    # Draw graph
    pos = nx.spring_layout(G)  # You can choose other layout algorithms
    nx.draw(G, pos, with_labels=True, font_weight='bold',node_color='dodgerblue',node_size=800)
    #yellow&dodgerblue
